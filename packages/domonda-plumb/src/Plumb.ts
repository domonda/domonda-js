...
export type Subscriber<T> =
  | ((state: Readonly<T>, tag: string | undefined) => void)
  | ({
      dispose?: () => void;
      next?: (state: Readonly<T>, tag: string | undefined) => void;
    });

export type Subscription = Disposable;

export type Transformer<T> = (state: Readonly<T>, tag: string | undefined) => T;

export type Filter<T> = (state: T, tag: string | undefined) => boolean;
...
export interface Plumb<T> extends Disposable {
  ...
  chain: <K>(props: ChainProps<T, K>, tag?: string) => Plumb<K>;
  next: (state: T, tag?: string) => void;
